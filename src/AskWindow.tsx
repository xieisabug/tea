...
function AskWindow() {
  ...
  const handleSubmit = () => {
    ...
    try {
      invoke<AiResponse>("ask_ai", {
        request: { prompt: query, conversation_id: "", assistant_id: 1 },
      }).then((res) => {
  ...
  const [shortcut, setShortcut] = useState<string>("CmdOrCtrl+Shift+I");

  useEffect(() => {
    invoke<string>("get_config", { key: "shortcut" }).then((shortcut) => {
      ...
      <div className="chat-container" data-tauri-drag-region>
        ...
        <div className="tools" data-tauri-drag-region>
          {messageId !== -1 && !aiIsResponsing ? (
            <IconButton
              icon={copySuccess ? <Ok fill="black" /> : <Copy fill="black" />}
              onClick={() => {
                writeText(response);
                setCopySuccess(true);
                setTimeout(() => {
